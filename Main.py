...
from ulits import Draw_Classification_Map,applyPCA,get_Samples_GT,GT_To_One_Hot
import copy
from test1 import trainx
...
for (FLAG, curr_train_ratio,Scale) in [(1,30,80)]: 
    ...
    for curr_seed in Seed_List:
        ...
        data_pca = applyPCA(data, numComponents )
        ...
        height, width, bands = data_pca.shape
        
        net_input=np.array( data_pca,np.float32)
        ...
        if dataset_name == "indian_":
            net = TFAP.TFAP(height, width, bands, class_count, Q, A, model='smoothed')
        else:
            net = TFAP.TFAP(height, width, bands, class_count, Q, A)
