...
class SLIC(object):
    ...
    def get_Q_and_S_and_Segments(self):
        ...
        segments = slic(img, n_segments=self.n_segments, compactness=self.compactness, max_iter=self.max_iter, # n_segments越小，分的块数量越少，越大，块越多
        ...
        out = mark_boundaries(img[:,:,[0,0,0]], segments) 
        ...
        for i in range(h - 2):
            for j in range(w - 2):
                ...
                if sub_max != sub_min:
                    ...
                    diss1 = np.exp(-0.1*np.dot(pix1-np.mean(pix1),pix2-np.mean(pix2))/(np.linalg.norm(pix1-np.mean(pix1))*np.linalg.norm(pix2-np.mean(pix2)))/ sigma ** 2) #皮尔逊相关系数

                    A[idx1, idx2] = A[idx2, idx1] = diss1
